defmodule SynaiPro.Email do
  ...
  An Swoosh email struct can be delivered by a Swoosh mailer (see mailer.ex & user_notifier.ex). Eg:
      ...
      SynaiPro.Email.confirm_register_email(user.email, url)
      |> SynaiPro.Mailer.deliver()
  """

  use Phoenix.Swoosh,
    view: SynaiProWeb.EmailView,
    layout: {SynaiProWeb.EmailView, :email_layout}
  ...
  defp from_name do
    SynaiPro.config(:mailer_default_from_name)
  end

  defp from_email do
    SynaiPro.config(:mailer_default_from_email)
  end
  ...
  def get_unsubscribe_url(user, email_kind) do
    SynaiPro.Accounts.NotificationSubscriptions.unsubscribe_url(user, email_kind)
  end
end
