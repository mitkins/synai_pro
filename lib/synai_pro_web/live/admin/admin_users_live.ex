defmodule SynaiProWeb.AdminUsersLive do
  @moduledoc """
  ...
  import SynaiProWeb.AdminLayoutComponent
  ...
  def apply_action(socket, :edit, %{"user_id" => user_id} = params) do
    ...
    |> assign(
      ...
      changeset: nil,
      page_title: "Editing #{SynaiProWeb.Helpers.user_name(user)}",
  ...
  def user_actions(assigns) do
    ...
    <div class="flex items-center" id={"user_actions_container_#{@user.id}"}>
      ...
      >
        ...
        <%= if SynaiPro.config(:impersonation_enabled?) && !@user.is_admin && @user.id != @current_user.id do %>
          <.dropdown_menu_item
            label="Impersonate"
            link_type="a"
