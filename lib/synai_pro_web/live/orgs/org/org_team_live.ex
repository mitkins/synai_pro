defmodule SynaiProWeb.OrgTeamLive do
  use SynaiProWeb, :live_view
  alias SynaiPro.Orgs
  alias SynaiPro.Repo

  import SynaiProWeb.OrgSettingsLayoutComponent

  on_mount {SynaiProWeb.OrgOnMountHooks, :require_org_admin}
  ...
  def handle_event("delete_member", %{"id" => id}, socket) do
    ...
    case Orgs.delete_membership(membership) do
      {:ok, membership} ->
        SynaiPro.Logs.log("orgs.delete_member", %{
        ...
        if membership.user_id == socket.assigns.current_user.id do
          {:noreply,
           ...
           |> push_navigate(to: SynaiProWeb.Helpers.home_path(socket.assigns.current_user))}
        else
          SynaiProWeb.UserAuth.disconnect_user_liveviews(member_user)
  ...
  def handle_event("delete_invitation", %{"id" => id}, socket) do
    ...
    SynaiPro.Logs.log("orgs.delete_invitation", %{
      user: socket.assigns.current_user,
      org_id: org.id,
      target_user_id: nil,
      metadata: %{
