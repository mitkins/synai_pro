defmodule SynaiProWeb.UserTOTPController do
  ...
  def create(conn, %{"user" => user_params}) do
    ...
    case Accounts.validate_user_totp(current_user, user_params["code"]) do
      :valid_totp ->
        SynaiPro.Logs.log("totp.validate", %{user: current_user})
      ...
      {:valid_backup_code, remaining} ->
        SynaiPro.Logs.log("totp.validate_with_backup_code", %{user: current_user})
      ...
      :invalid ->
        SynaiPro.Logs.log("totp.invalid_code_used", %{user: current_user})
  ...
  defp redirect_if_totp_is_not_pending(conn, _opts) do
    if get_session(conn, @pending) do
      ...
      conn
      |> redirect(to: SynaiProWeb.Helpers.home_path(conn.assigns.current_user))
      |> halt()
    end
  end
end
