defmodule SynaiPro.Logs do
  ...
  def create(attrs \\ %{}) do
    case %Log{}
         ...
         |> Repo.insert() do
      {:ok, log} ->
        SynaiProWeb.Endpoint.broadcast("logs", "new-log", log)
        {:ok, log}
  ...
  def log_async(action, params) do
    SynaiPro.BackgroundTask.run(fn ->
      log(action, params)
    end)
  end
  ...
  Examples:
      ...
      SynaiPro.Logs.log("orgs.create_invitation", %{
  ...
  def exists?(params) do
    ...
    |> SynaiPro.Repo.exists?()
  end

  def get_last_log_of_user(user) do
    SynaiPro.Logs.LogQuery.by_user(user.id)
