defmodule SynaiPro.FileUploads.Cloudinary do
  @moduledoc """
  ...
  def update(assigns, socket) do
    {:ok,
     ...
     |> allow_upload(:image,
       accept: ~w(image/*),
       max_entries: 3,
       external: &SynaiPro.FileUploads.Cloudinary.presign_upload/2
     )}
  ```
  ...
  defp file_id_to_url(public_id) do
    cloud_name = SynaiPro.config([:cloudinary, :cloud_name])
    folder = SynaiPro.config([:cloudinary, :folder])
  ...
  defp sign(config) do
    ...
    secret = SynaiPro.config([:cloudinary, :api_secret])
    api_key = SynaiPro.config([:cloudinary, :api_key])
  ...
  defp maybe_put_folder(config) do
    case SynaiPro.config([:cloudinary, :folder]) do
      nil -> config
  ...
  defp url,
    do:
      Path.join([
        ...
        SynaiPro.config([:cloudinary, :cloud_name]),
        "image/upload"
      ])
end
