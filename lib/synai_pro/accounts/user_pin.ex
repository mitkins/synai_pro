defmodule SynaiPro.Accounts.UserPin do
  use Ecto.Schema
  ...
  schema "users_pins" do
    ...
    belongs_to :user, SynaiPro.Accounts.User
  ...
  defp validation_status(%__MODULE__{} = user_pin, pin, allowed_attempts) do
    expired? = Timex.diff(Timex.now(), user_pin.inserted_at, :minutes) >= @pin_validity_in_minutes
  ...
  defp get_hashed_pin(pin),
    do:
      :crypto.mac(
        :hmac,
        :sha256,
        Application.get_env(:synai_pro, SynaiProWeb.Endpoint)[:secret_key_base],
        pin
      )
