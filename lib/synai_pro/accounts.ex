defmodule SynaiPro.Accounts do
  @moduledoc """
  ...
  def user_lifecycle_action("after_register", user, %{registration_type: registration_type}) do
    SynaiPro.Logs.log_async("register", %{
      user: user,
      metadata: %{registration_type: registration_type}
    })

    SynaiPro.Orgs.sync_user_invitations(user)

    SynaiPro.Slack.message("""
    :bust_in_silhouette: *A new user joined!*
    *Name*: #{SynaiProWeb.Helpers.user_name(user)}

    #{SynaiProWeb.Router.Helpers.admin_users_url(SynaiProWeb.Endpoint, :edit, user)}
    """)
  end

  def user_lifecycle_action("after_confirm_email", user, _) do
    SynaiPro.Logs.log_async("confirm_email", %{user: user})
    SynaiPro.Orgs.sync_user_invitations(user)
  end

  def user_lifecycle_action("after_sign_in", user, %{ip: ip}) do
    SynaiPro.Logs.log_async("sign_in", %{user: user})
    {:ok, user} = update_last_signed_in_info(user, ip)
    SynaiPro.MailBluster.sync_user_async(user)
  end
  ...
  def user_lifecycle_action("after_impersonate_user", user, %{
    ...
    SynaiPro.Logs.log_async("impersonate_user", %{user: user, target_user_id: target_user_id})
  ...
  def user_lifecycle_action("after_restore_impersonator", user, %{
    ...
    SynaiPro.Logs.log_async("restore_impersonator", %{user: user, target_user_id: target_user_id})
    update_last_signed_in_info(user, ip)
  end

  def user_lifecycle_action("after_update_profile", user, _) do
    SynaiPro.Logs.log_async("update_profile", %{user: user})
    SynaiPro.MailBluster.sync_user_async(user)
  end

  def user_lifecycle_action("after_confirm_new_email", user, _) do
    SynaiPro.Logs.log_async("confirm_new_email", %{user: user})
    SynaiPro.Orgs.sync_user_invitations(user)
  end

  def user_lifecycle_action("request_new_email", user, %{new_email: new_email}) do
    SynaiPro.Logs.log_async("request_new_email", %{user: user, metadata: %{new_email: new_email}})
  end

  def user_lifecycle_action("after_passwordless_pin_sent", user, %{pin: pin}) do
    SynaiPro.Logs.log("passwordless_pin_sent", %{user: user})
