defmodule SynaiProWeb.UserAuthTest do
  use SynaiProWeb.ConnCase, async: true

  alias SynaiPro.Accounts
  alias SynaiProWeb.UserAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, SynaiProWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "redirects to org invites if an invite is pending", %{conn: conn, user: user} do
      org = SynaiPro.OrgsFixtures.org_fixture()
      SynaiPro.OrgsFixtures.invitation_fixture(org, %{email: user.email})
      conn = UserAuth.log_in_user(conn, user)
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      SynaiProWeb.Endpoint.subscribe(live_socket_id)
