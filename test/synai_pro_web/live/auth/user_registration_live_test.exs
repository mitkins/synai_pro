defmodule SynaiProWeb.UserRegistrationLiveTest do
  ...
  describe "Registration page" do
    ...
    test "redirects if already logged in", %{conn: conn} do
      ...
      result =
        ...
        |> follow_redirect(conn, SynaiProWeb.Helpers.home_path(user))
  ...
  describe "register user" do
    test "creates account and logs the user in", %{conn: conn} do
      ...
      user = SynaiPro.Repo.last(SynaiPro.Accounts.User)
      assert redirected_to(conn) == SynaiProWeb.Helpers.home_path(user)

      # Now do a logged in request and assert on the menu
      conn = get(conn, SynaiProWeb.Helpers.home_path(user))
