defmodule SynaiProWeb.EditProfileLiveTest do
  ...
  describe "when signed in" do
    ...
    test "event update_profile works", %{conn: conn, user: user} do
      ...
      user = SynaiPro.Accounts.get_user!(user.id)
      assert user.name == "123456789"

      log = Repo.last(SynaiPro.Logs.Log)
      assert log.user_id == user.id
      assert log.action == "update_profile"
    end

    test "user can remove their avatar", %{conn: conn, user: user} do
      {:ok, user} = SynaiPro.Accounts.update_profile(user, %{avatar: "avatar"})
