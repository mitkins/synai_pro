defmodule SynaiProWeb.PasswordlessAuthLiveTest do
  ...
  describe "no existing user" do
    test "it creates a new user and logs them in", %{conn: conn} do
      inital_user_count = Repo.count(SynaiPro.Accounts.User)
      ...
      assert inital_user_count + 1 == Repo.count(SynaiPro.Accounts.User)
      new_user = Repo.last(SynaiPro.Accounts.User)
      ...
      sign_in_token =
        new_user
        |> SynaiPro.Accounts.generate_user_session_token()
        |> Base.encode64()

      form = form(view, "form", %{"auth" => %{"sign_in_token" => sign_in_token}})
      conn = follow_trigger_action(form, conn)
      assert redirected_to(conn) =~ SynaiProWeb.Helpers.home_path(new_user)
  ...
  describe "with existing user" do
    test "it logs in the existing user", %{conn: conn} do
      ...
      conn = follow_trigger_action(form, conn)
      assert redirected_to(conn) =~ SynaiProWeb.Helpers.home_path(user)
      assert inital_user_count == Repo.count(SynaiPro.Accounts.User)
    end
  end
end
